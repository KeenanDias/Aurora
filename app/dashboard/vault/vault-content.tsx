  ...
  return (
    <>
        ...
        >
          <div className="flex items-center gap-3 mb-3">
            ...
            >
              <Link2
                className={`w-5 h-5 ${
                  plaidConnected ? "text-emerald-400" : "text-white/30"
                }`}
              />
            </div>
            <div>
      ...
      <button
        ...
        className="w-full mb-8 py-4 rounded-xl border-2 border-dashed border-white/10 hover:border-teal-500/30 bg-white/[0.02] hover:bg-teal-500/[0.03] transition-all flex items-center justify-center gap-3 group disabled:opacity-50"
      >
        <Upload className="w-5 h-5 text-white/30 group-hover:text-teal-400 transition-colors" />
      ...
      {uploads.length > 0 && (
        <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-4 mb-8">
          <div className="flex items-center gap-2 mb-3">
            ...
            <p className="text-xs font-medium text-white/50">Security Ledger</p>
          </div>
      ...
      {loading ? (
        <div className="text-center py-12">
          ...
          <p className="text-xs text-white/30 mt-3">Loading vault...</p>
        </div>
      ) : uploads.length === 0 ? (
        <div className="text-center py-16">
          <FileText className="w-12 h-12 text-white/10 mx-auto mb-4" />
          <p className="text-sm text-white/40 mb-2">No statements uploaded yet</p>
        ...
        <div className="rounded-xl border border-white/[0.06] overflow-hidden">
          <div className="grid grid-cols-[1fr_120px_120px_80px_80px_50px] gap-4 px-5 py-3 border-b border-white/[0.06] bg-white/[0.02]">
            <span className="text-xs font-medium text-white/40">File</span>
            <span className="text-xs font-medium text-white/40">Period</span>
            <span className="text-xs font-medium text-white/40">Source</span>
            <span className="text-xs font-medium text-white/40 text-right">Spending</span>
            <span className="text-xs font-medium text-white/40 text-center">Details</span>
            <span className="text-xs font-medium text-white/40 text-center"></span>
          </div>
          {uploads.map((u) => (
            <div
              key={u.id}
              className="grid grid-cols-[1fr_120px_120px_80px_80px_50px] gap-4 px-5 py-3.5 border-b border-white/[0.04] hover:bg-white/[0.02] transition-colors items-center"
            >
              <div className="flex items-center gap-2 min-w-0">
                <FileText className="w-4 h-4 text-violet-400 shrink-0" />
                <span className="text-sm text-white/70 truncate">{u.filename}</span>
              </div>
        ...
        {(selectedUpload || detailLoading) && (
          ...
          >
            <motion.div
              ...
              className="w-full max-w-2xl max-h-[80vh] rounded-2xl border border-white/10 bg-[#0b1120] overflow-hidden flex flex-col"
            >
              {detailLoading ? (
                ...
                <>
                  ...
                  <div className="grid grid-cols-3 gap-4 p-5 border-b border-white/[0.04]">
                    ...
                    <div>
                      <p className="text-xs text-white/40">Spending</p>
                      <p className="text-lg font-bold text-orange-400">
                        ${selectedUpload.total_spending.toLocaleString()}
                      </p>
                    </div>
                    <div>
                      <p className="text-xs text-white/40">Fixed Bills</p>
